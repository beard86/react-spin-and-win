...
const authCheck = jwt({
  secret: new Buffer('kA-KNtkmD0HvOz-cr6NXJekx4Elt2jtFWsJbkDBd6DML1U-tkuAmcmTNKtcsEq4N', 'base64'),
  audience: '3RhBq3qZaZARDQae7PtbH59wyP9xe7Ld'
});
...
app.get('/api/contacts/:id', authCheck, (req, res) => {
  console.log(app,authCheck,contacts)
  res.json(contacts.filter(contact => contact.id === parseInt(req.params.id))[0]);
});

app.listen(3001);
console.log(authCheck,'Listening on http://localhost:3001');